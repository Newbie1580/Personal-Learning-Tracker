...
<div class="mx-auto max-w-2xl">
    ...
    <div class="anim-rise mt-3 flex items-center gap-4" style="animation-delay:60ms">
        ...
        <div>
            <h1 class="text-3xl font-semibold tracking-tight text-stone-900">Create Skill</h1>
            <p class="text-sm text-stone-500">Add a skill and set your starting level.</p>
        </div>
    </div>

    <form method="POST" action="{{ route('skills.store') }}" class="anim-rise mt-6 overflow-hidden rounded-3xl border border-stone-900/10 bg-white shadow-sm" style="animation-delay:120ms">
        @csrf
        <div class="space-y-5 p-6 sm:p-8">
            <div>
                <label class="mb-1.5 block text-sm font-bold text-stone-700">Name <span class="text-orange-700">*</span></label>
                <input type="text" name="name" value="{{ old('name') }}" required maxlength="255" placeholder="e.g. Eloquent ORM" class="w-full rounded-xl border-0 bg-stone-100 px-4 py-2.5 text-sm font-medium text-stone-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-orange-700 @error('name') ring-2 ring-red-700 @enderror">
                ...
                <div class="grid grid-cols-3 gap-2">
                    @foreach ($levels as $level)
                        <label class="cursor-pointer">
                            <input type="radio" name="proficiency_level" value="{{ $level }}" @checked(old('proficiency_level', 'beginner') === $level) class="peer sr-only">
            ...
            <div>
                ...
                <textarea name="description" rows="3" placeholder="What does this skill cover?" class="w-full rounded-xl border-0 bg-stone-100 px-4 py-2.5 text-sm text-stone-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-orange-700">{{ old('description') }}</textarea>
            </div>
        </div>
        <div class="flex flex-col-reverse gap-2 border-t border-stone-100 bg-stone-50 px-6 py-4 sm:flex-row sm:justify-end sm:px-8">
            ...
            <button class="inline-flex items-center justify-center gap-1.5 rounded-xl bg-orange-700 px-6 py-2.5 text-sm font-bold text-amber-50 shadow-md transition hover:bg-orange-800"><i data-lucide="check" class="h-4 w-4"></i>Save skill</button>
        </div>
    </form>
</div>
@endsection
